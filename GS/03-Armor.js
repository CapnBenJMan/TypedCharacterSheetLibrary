...
class Armor {
    constructor(name, ACText, ACFormula, strReq, stealth) {
        this.name = name;
        this.ACText = ACText;
...
function armorSetter(selection, armor, stat, natStat, shield, bonuses, notes, custom) {
    ...
    const selected = (armor.toLowerCase() != 'custom') ?
        ...
        new Armor(custom.name, custom.baseAC.toString() + ((custom.plusDex) ? ' + Dex modifier' : '') + ((custom.dexMax > 0) ? ` (max ${custom.dexMax})` : ''), `=${custom.baseAC}${(custom.plusDex) ? ((custom.dexMax > 0) ? `+min(Dex, ${custom.dexMax})` : '+Dex') : ''}`, (custom.minStr > 1) ? `Str ${custom.minStr}` : '-', (custom.disStealth) ? 'Disadvantage' : '-'); /*  {
        name: ,
