...
function addEditInfo(className, subclass, level, hitdie, spells, x, selection) {
    ...
    function checkDuplicate(name) {
        for (const a of classValues) { // loop through classValues
            if (name.toLowerCase().replace(/ /g, "") == a.toLowerCase().replace(/ /g, ""))
                return true;
...
function adjustExpended(ss) {
    ...
    for (let i = 1; i <= 4; i++) { // loop through each column
        if (cell(current, 1, i) > cell(count, 1, i))
            current.getCell(1, i).setValue(cell(count, 1, i));
...
function adjustNote(ss) {
    ...
    if (note != noteBuilder)
        classRange.setNote(noteBuilder);
...
function adjustNamedRanges(ss) {
    ...
    for (let i = 1; i <= classes.getNumRows(); i++) { // loop through class range
        ...
        if (ss.getRangeByName(`${current}Lvl`) == null && !isEmptyish(current))
            ss.setNamedRange(`${current}Lvl`, classes.getCell(i, 3));
 ...
 */
function doSpells(ss, Class, Subclass, spells) {
    if (Class == "")
        return; // ends execution if className is blank
