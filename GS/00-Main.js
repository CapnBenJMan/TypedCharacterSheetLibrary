...
function version(i) {
    ...
    const rangeLBV = i.source.getRange("Legal and How-to!AC5");
...
function trigger(e, id) {
    ...
    if (id == "1IOzfu9LPV-Vv3IJ3caolxHw-W1YJiyYAluXDfmbPiFs" && false) { // You are currently passing "0" in the test sheet
        ...
        switch (name) {
            ...
            default: // else
                if (sheetNames.includes(name))
                    spells(e); // run spells code if the current sheet is a spells sheet
                break;
        }
    }
    else {
        switch (name) {
            ...
            default: // else
                if (sheetNames.includes(name))
                    spells(e); // run spells(e) if current sheet is a spells sheet
                break;
        }
    }
}
function preload(i, id) {
    sideBar(); // open sidebar
    version(i); // update version
    if (id == "1IOzfu9LPV-Vv3IJ3caolxHw-W1YJiyYAluXDfmbPiFs")
        jsonSwap({
            source: i.source,
            value: "Page 1",
