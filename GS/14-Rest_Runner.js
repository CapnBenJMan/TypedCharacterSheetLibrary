...
function restRunner(type) {
    ...
    function inputvalRunner(range, title, snippet) {
        ...
        if (resB == ui.Button.CANCEL)
            return; // ends execution if CANCEL button was pressed
        else if (resB == ui.Button.OK)
            ss.getRange(range).setValue(resT); // otherwise, sets inputted text to range
    }
    /** Sets a random value from a list based on a previously inputted set of values
    */
    function randomlistRunner(range, args) {
    ...
    */
    ...
    function modinputRunner(range, title, snippet) {
        while (true) {
            const res = ui.prompt(title, `Enter the number you wish to modify the current value of a cell by (begin with + or -) or enter 0 to make no change\n` +
                `Your snippet is as follows:\n${snippet}`, ui.ButtonSet.OK_CANCEL); // create a prompt for the user to enter a value into
    ...
    function srreminderRunner(range, title, snippet) {
