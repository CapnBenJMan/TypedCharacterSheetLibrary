 ...
 */
function restCompiler(range, type, sr, rest) {
    const ss = SpreadsheetApp.getActiveSpreadsheet();
    const ui = SpreadsheetApp.getUi();
    const restListSheet = ss.getSheetByName('Rest List');
    var lrow = restListSheet.getLastRow();
    if (lrow == 0) {
        lrow = 1;
    }
    const rangelist = restListSheet.getRange(1, 1, lrow).getValues().flat();
    var row = restListSheet.getLastRow() + 1;
    if (rangelist.includes(range))
        row = rangelist.indexOf(range) + 1;
    const infoArray = [`="${range}"`, '', type, '', true, sr];
    var returnTrue;
    switch (type) {
    ...
    if (returnTrue == 'return')
        return; // prevents an incomplete rule from being added
    restListSheet.getRange(row, 1, 1, 6).setValues([infoArray]); // adds the rule to the proper range
    function cellCase() {
        // var sheet, cell
        try {
            const res = ui.prompt('Enter the range of the cell you want to reference. (Ex. A1, Character!B2)\n' +
    ...
    function inputvalCase() {
        const resA = ui.prompt(`Enter the title of this selection. After entering the title, write a brief snippet detailing what is requiring the input.\n` +
            `Separate the title and snippet with a bar ("|").`); // prompts the user for input
    ...
    function otherCase() {
        var res = ui.prompt(`Enter the value you want this cell to be reset to on a ${rest} rest.\n` +
            'When entering a boolean (true/false) value, use all caps (ie. TRUE, FALSE).', ui.ButtonSet.OK_CANCEL);
        var resT = res.getResponseText();
        var resB = res.getSelectedButton();
    ...
    function randomnumCase() {
        try {
            const res = ui.prompt(`Enter the random number to be generated in the form of a dice roll (ie. X1dY1+Z1 X2dY2+Z2 etc.).\n` +
                `X = Number of a type of dice\n` +
                `Y = Type of dice\n` +
                `Z = Number to add to the roll\n` +
                `Ex. 1d4-1 2d6+2 3d3\n` +
                `Important: Be sure to use the formatting above, or else the code will get mad.`).getResponseText();
            infoArray[3] = res.split(" ") // 
                ...
                const m = cur.match(/(\d+)d(\d+)([+-]\d+)?/i) // find matches to the format of XdY+-Z
                    ...
                    .map((x, i) => i < 2 ? Number(x) : x); // make sure each of the first 2 items is a number
    ...
    function randomlistCase() {
        ...
        if (cellVal == null) { // if cell has no data validations
            ...
            var but = ui.alert(// ...alert the user of this and ask if they would like to use it
            `The cell you have selected contains a dropdown list. ` +
                `Would you like to have the code use the values of this list (Yes) ` +
                `or would you like to enter your own (No)?`, ui.ButtonSet.YES_NO_CANCEL);
            switch (but) {
                case ui.Button.YES:
    ...
    function modconstantCase() {
        const input = ui.prompt(`Please enter the number you want this cell to be modified by on a ${rest} rest.\n` +
            `For adding, start with a "+". For subtracting, start with a "-"`).getResponseText(); // get user input
