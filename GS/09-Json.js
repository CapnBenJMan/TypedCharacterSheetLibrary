function jsonSwap(e) {
    ...
    console.log("JSON");
    if (!isEmptyish(e))
        console.log(e.range.getA1Notation().replace(/:.+/, ""));
    ...
    const obj = [
        ...
        if (a1 == 'Z57' || a1 == 'AH57')
            return x.inputRange.includes(a1) && x.name.includes(mode);
    ...
    // Load JSON
    const content = JSON.parse(contentLoadpoint.getValue());
    const formatting = JSON.parse(formatLoadpoint.getValue());
    ...
    const saveValis = range.getDataValidations().map(x => x.map(y => {
        if (y == null)
            return null;
        const critType = y.getCriteriaType().toString(), critValues = y.getCriteriaValues();
        return [critType,
