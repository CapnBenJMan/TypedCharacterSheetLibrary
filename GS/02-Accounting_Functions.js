...
/* function coinClass() { // the source of the CoinSet class
        ...
        try {
            if (arguments.length > 0) { // if container is not undefined
                ...
                // ^if container is an array, return raw values in an array
                    ...
                    throw "Error: Your container is not an object or array.";
                // ^else throw an error
            }
            else
                return this.coins.map(x => x.raw);
    ...
    getPoints(container) {
        try {
            if (arguments.length > 0) { // if container is not undefined
                ...
                return this.points; // else return points property
        }
        catch (error) {
            console.error(error);
        } // log error
    ...
    getChange(container) {
        try {
            if (arguments.length > 0) { // if container is not undefined
                if (Array.isArray(container))
                    return this.coins.map(x => x.change);
    ...
    copy() {
        ...
        this.plat.raw, this.gold.raw, this.silv.raw, this.copp.raw, this.plat.change, this.gold.change, this.silv.change, this.copp.change);
    }
    /**
     ...
     */
    static fromRaw(container, changer = [0, 0, 0, 0]) {
        try {
            const coinset = new CoinSet(); // define new coinset
            const change = []; // define storage for change values
            if (!Array.isArray(changer) && typeof changer == "object")
                for (const [_key, value] of Object.entries(changer))
                    change.push(Number(value));
            // ^if changer is not an array and changer is an object, loop through the entries and push them to change
            else
                changer.forEach(x => change.push(Number(x))); // otherwise loop through changer and push the values to change
    ...
    distribute({ coppMinDigits: cMD, silvMinDigits: sMD, goldMinDigits: gMD, platMinDigits: pMD } = {}) {
        const coppMinDigits = cMD !== null && cMD !== void 0 ? cMD : 1;
        const silvMinDigits = sMD !== null && sMD !== void 0 ? sMD : 1;
        const goldMinDigits = gMD !== null && gMD !== void 0 ? gMD : 1;
        const platMinDigits = pMD !== null && pMD !== void 0 ? pMD : 1;
        try {
            ...
            if (this.points < 0)
                throw `Error: You don't have enough coin to complete this transaction.` +
            ...
            while (true) { // keep looping until broken
                const bools = {
                    copp: {
                        fromSilv: this.silv.raw > silvMin,
                        fromGold: this.gold.raw > goldMin,
                        fromPlat: this.plat.raw > platMin // ...platinum
                    },
                    silv: {
                        fromCopp: this.copp.raw > (9 + coppMin),
                        fromGold: this.gold.raw > goldMin,
                        fromPlat: this.plat.raw > platMin // ...platinum
                    },
                    gold: {
                        fromCopp: this.copp.raw > (99 + coppMin),
                        fromSilv: this.silv.raw > (9 + silvMin),
                        fromPlat: this.plat.raw > platMin // ...platinum
                    },
                    plat: {
                        fromCopp: this.copp.raw > (999 + coppMin),
                        fromSilv: this.silv.raw > (99 + silvMin),
                        fromGold: this.gold.raw > (9 + goldMin) // ...gold
                    }
                };
                const rawVals = [this.copp.raw, this.silv.raw, this.gold.raw, this.plat.raw];
                if (Date.now() - start > 1500 && rawVals.every(x => x >= 0))
                    throw "time";
                ...
                if (this.copp.raw.toString().length < coppMinDigits || this.copp.raw < coppMin) {
                    ...
                    if (bools.copp.fromSilv) { // if copper can be made from silver
                        ...
                        this.copp.change += 10; // ^^^
                        if (checker())
                            break; // break if checker returns true
                    }
            ...
            if (err == "time") {
                console.warn("Either took too long or timed out intentionally");
            } // log warning
            else {
                console.error(err);
            } // otherwise log error
...
function runManualDistributor(arr) {
