...
function addLongRest() {
    ...
    if (sheet != "Rest List") { // if current cell's sheet is not rest list
        ...
        if (!ranges.flat().includes(rangeP))
            ...
            const yn = ui.alert("Warning: the cell you wish to format already has a rule set to it. " +
...
function getHitDice() {
    const ss = SpreadsheetApp.getActiveSpreadsheet(); // define spreadsheet reference
    /** @returns Cell Value */
    const val = (r) => Number(ss.getRange(r).getValue()); // arrow function to get value of a range and return it as a number
    return {
...
function updateHitDice(d6 = 0, d8 = 0, d10 = 0, d12 = 0, bool = false, plus = 0, initial = []) {
    const ss = SpreadsheetApp.getActiveSpreadsheet(); // define spreadsheet reference
    const setVal = (range, value) => ss.getRange(range).setValue(value); // arrow function to set a value to a specific range
    for (const range of [["Character!AQ6", d6], ["Character!AR6", d8], ["Character!AS6", d10], ["Character!AT6", d12]])
