...
function shortRest() {
    ...
    if (ss.getRange(`Character!AQ6:AT6`).getValues().flat().some(x => Number(x) > 0))
        openHTML('shortrest');
    ...
    for (let x in spls)
        spls[x].setValue(splsVals[x]); // set PM ranges to their max values (reset them to full)
...
function addLongRest() {
    ...
    if (sheet != 'Rest List') { // if current cell's sheet is not rest list
        ...
        if (!ranges.flat().includes(rangeP))
            openHTML('lrdialog'); // if ranges doesn't include rangeP, show the dialog
        else if (ranges.flat().includes(rangeP)) { // else if it does include rangeP
            var yn = ui.alert('Warning: the cell you wish to format already has a rule set to it. ' +
                'Proceeding will overwrite this rule.\n' +
                'Do you wish to proceed?', ui.ButtonSet.OK_CANCEL); // alert user of existing rule on cell
            if (yn == ui.Button.OK)
                openHTML('lrdialog'); // if user pressed okay button show dialog
        }
    }
    else
        ui.alert('Do not try to add a long rest here.'); // otherwise alert user of restriction
}
...
function removeLongRest() {
    ...
    if (ranges.includes(rangeP)) { // if ranges includes rangeP
        ...
        if (res == ui.Button.OK) { // if button selection is okay
            if (lastRow == 1)
                restListSheet.getRange(1, 1, 1, restListSheet.getLastColumn()).clearContent();
            // ^clear row content if last row is 1
            else if (lastRow > 1)
                restListSheet.deleteRow(ranges.indexOf(rangeP) + 1);
...
function adjustHitDice(ss) {
    ...
    const numExpended = expendedVals[0].reduce((total, val) => total + Number(val), 0);
    // ^defines the total number of expended hit dice a character has
    if ((numExpended || 1) >= Math.ceil(numHitDice / 2))
        expendedRange.setValues(hitDiceVals);
    ...
    // ^^set expended hit dice to their respective maxxes
        ...
        var numZero = 0, pos = 0; // define numZero and pos
        hitDiceVals[0].forEach((x, i) => {
            if (Number(x) == 0)
                numZero++; // if x is 0, increment numZero
            else
                pos = i; // otherwise, set pos to i
        });
        if (numZero == 3) { // if there is only 1 type of hit die available
            ...
            openHTML('adjuster'); // otherwise open adjuster dialog
    }
}
function getHitDice() {
    const ss = SpreadsheetApp.getActiveSpreadsheet(); // define spreadsheet reference
    /** @returns Cell Value */
    let val = (r) => Number(ss.getRange(r).getValue()); // arrow function to get value of a range and return it as a number
    return {
...
function updateHitDice(d6 = 0, d8 = 0, d10 = 0, d12 = 0, bool = false, plus = 0, initial = []) {
    const ss = SpreadsheetApp.getActiveSpreadsheet(); // define spreadsheet reference
    /** @param {string} range */
    let setVal = (range, value) => ss.getRange(range).setValue(value); // arrow function to set a value to a specific range
    for (let range of [['Character!AQ6', d6], ['Character!AR6', d8], ['Character!AS6', d10], ['Character!AT6', d12]])
    ...
    if (bool) { // if bool is set to true
        ...
        const currentHpVal = Math.min(Math.max(Number(ss.getRange("Character!R17").getValue()) +
