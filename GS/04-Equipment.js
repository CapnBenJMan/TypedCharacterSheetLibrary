...
function setEquipment(category, name) {
    ...
    const GR = (n) => ss.getRange(n); // define arrow function to easily get a range
    try {
        const sheetName = cell.getSheet().getName(); // define the sheet name of the current cell
        if (["Character", "Storage"].every(x => x != sheetName))
            throw "sheet";
        ...
        if (![char, sto1, sto2, sto3, sto4].some(x => x))
            throw "range";
        // ^end execution if cell is not in the proper range
        const obj = EquipmentInfo[category].find(x => x.Name == name);
        ...
        if (check(obj)) { // if item is an equipment pack
            ...
            if (!targetRange.isBlank())
                throw "blank"; // if the range is not blank, throw error
            if (obj["Contents"].length + row > limit)
                throw "oob"; // throw error if out of bounds
            // 15 wide
            ...
            for (let a = 0; a < obj["Contents"].length; a++) { // loop through contents
                ...
                const adj1 = (item["Weight"] != "-") ? item["Weight"].replace(/[^\d\/\.]/g, "") : "-";
                ...
                if ("Note" in item)
                    arr1[1] = item["Note"]; // add a note for the item if it has one
            ...
            // ^define reference to target range
            if (!targetRange.isBlank())
                throw "blank"; // throw error if the target range is not blank
            targetRange.setValues([[obj["Quantity"], obj["Name"], "", "", "", "", "", "", "", obj["Cost"].replace("—", "-"), "", "", weight, "", ""]]);
            // ^set values of item to target range
        }
        if ("Note" in obj)
            ss.getSheetByName(sheetName).getRange(row, colStart + 1).setNote(obj["Note"]);
