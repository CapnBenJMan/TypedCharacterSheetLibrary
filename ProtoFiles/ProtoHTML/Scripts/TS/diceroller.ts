...
document.addEventListener("DOMContentLoaded", async () => { // on DOM Content Loaded
	try { await (async () => stats = await runGoogleWithReturn("getStats"))() } catch {
		// continue regardless of error
	} // try to get the stats from the sheet
...
function pageTurner(evt: Event, page: string) {
	...
	switch (curPage.id) {
		...
		case "Misc Dice":
			...
			dice.forEach(die => {
				const els = Array.from(qryA("svg polygon,polyline", die)) as SVGElement[]
	...
	for (const x of tabcontent) x.style.display = "none"
	const tablinks = Array.from(document.getElementsByClassName("tablinks")) as HTMLButtonElement[]
	for (const x of tablinks) x.classList.toggle("active", false)
...
function multiroller() {
	...
	for (const x of dx) (Number(ID<Input>(`numd${x}`).value) > 0 ? dns : other).push(ID(`d${x}`))
	if (other.length) resetMiscDice(other.map(x => Number(x.id.slice(1))), false)
	if (dns.length > 0) {
		const proms = dns.map(f => runSubAnimation(f))
		Promise.all(proms).then(() => {
			for (const j of dx) grandTotal.totals.push(Number(ID(`d${j}total`).innerHTML.slice(7)))
