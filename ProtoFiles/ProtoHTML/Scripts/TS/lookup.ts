...
const urlFormatter = (n: string) => n.replace(/'/g, ``).replace(/\/| /g, `-`).toLowerCase()
// ^arrow function for formatting urls

function changer() {
	if (String(ID<Select>("searchoptions").value) != "class") qry("form").style.display = "flex"
	else qry("form").style.display = "block"
	ID("variance").innerHTML = searchOptions[String(ID<Select>("searchoptions").value)]
	try {
		const ih = ID<Select>("selector").innerHTML
			.split(/(?<=>)(?:\n|\t)+?(?=<)/g)
			.map(x => x.replace(/(?:\n|\t)/g, ""))
			.sort((a0, b0) => {
		...
		ID<Select>("selector").innerHTML = ih
	} catch {
...
function classChange() {
	const classval = String(ID<Select>("classSearch").value)
	if (classval == "sub") {
		ID("subsearchdiv").className = ""
		ID<Input>("subsearch").required = true
	} else {
		ID("subsearchdiv").className = "magic"
		ID<Input>("subsearch").required = false
	}
}
...
async function handler(e: Event) {
	...
	try {
		...
		const search = ID<Input | Select>(searches[ID<Select>("searchoptions").value]).value == "level" ?
			Number(ID<Input>("searchbar").value.match(/\d+/)) :
			searches[ID<Select>("searchoptions").value] == "selector" ?
				String(ID<Select>("selector").value) : String(ID<Input>("searchbar").value)
		let baseURL = "http://dnd5e.wikidot.com"
		const Regex: { [k: string]: RegExp } = {
			prettier: /(?:<\/?a.*?>|(?:id|class)=".+?"|<script(?:.|\n)+?<\/script>)/g
		}
		switch (String(ID<Select>("searchoptions").value)) {
			case "class":
				const Class = urlFormatter(String(ID<Input>("classname").value))
				const subclass = urlFormatter(String(ID<Input>("subclassname").value))
				const subsearch = urlFormatter(String(ID<Input>("subsearch").value))
				baseURL += `/${Class}`
				if (subclass != "") baseURL += `:${subclass}`
				if (subsearch != "" && String(ID<Select>("classSearch").value) == "sub") baseURL += `:${subsearch}`
				Regex.featureNamePt1 = new RegExp(`<span>${(String(ID<Select>("classSearch").value) == "name") ? capitalizer(String(search)) :
					""}<\\/span><\\/h(?<num>\\d)>\\n<p>(?:.|\\n)*?(?=\\n<h\\k<num>)`, "i")
				Regex.featureNamePt2 = new RegExp(`(?<=<span>${(String(ID<Select>("classSearch").value) == "name") ? capitalizer(String(search)) :
					""}<\\/span><\\/h)\\d+(?=>)`, "")
				...
				Regex.featureSubPt1 = new RegExp(
					`(?:<span.*?>){1,2}${capitalizer(String(search))}(?:<\\/span>\\n?\\t*?){1,2}<\\/h(?<num>\\d)>(?:.|\\n)*?(?=(?:\\n<h\\k<num>|\\n?\\t*?<\\/div>))`,
					"i")
				Regex.featureSubPt2 = new RegExp(`(?<=<\\/span><\\/h)\\d+(?=>)`, "")
				break
			case "feat":
				baseURL += `/feat:${urlFormatter(String(ID<Input>("searchbar").value))}`
				break
			case "item":
				baseURL += `/wondrous-items:${urlFormatter(String(ID<Input>("searchbar").value))}`
				break
			case "spell":
				baseURL += `/spell:${urlFormatter(String(ID<Input>("searchbar").value))}`
				break
			case "race":
				baseURL += `/${urlFormatter(String(ID<Input>("searchbar").value))}`
				break
			case "background":
				baseURL += `/background:${urlFormatter(String(ID<Input>("searchbar").value))}`
				break
			case "misc":
				baseURL += `/${ID<Select>("selector").value}`
				break
		}
		let response0: string, bo = false
		try {
			response0 = await runGoogleWithReturn("returnFetch", [baseURL])
		} catch {
			try {
				if (String(ID<Select>("searchoptions").value) == "spell") throw ""
				baseURL += "s"
				bo = true
				response0 = await runGoogleWithReturn("returnFetch", [baseURL])
			} catch {
		...
		if (String(ID<Select>("searchoptions").value) == "misc" && String(ID<Select>("selector").value) != "trinkets") {
			try { qryA(".yui-nav", el).forEach(x => x.remove()) } catch { /* continue regardless of error */ }
			try { qryA(".yui-content>div", el).forEach(x => x.style.display = "") } catch { /* continue regardless of error */ }
		}
		if (search != "") {
			switch (String(ID<Select>("searchoptions").value)) {
				case "class":
				...
				case "misc":
					...
					const title = qry(`option[value="${ID<Select>("selector").value}"`).innerHTML
		...
		} else if (String(ID<Select>("searchoptions").value) == "class")
