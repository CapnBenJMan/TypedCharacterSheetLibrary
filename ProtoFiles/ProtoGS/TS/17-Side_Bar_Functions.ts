...
function isWithinRange(subRange: GoogleAppsScript.Spreadsheet.Range, ...searchRange: GoogleAppsScript.Spreadsheet.Range[]) {
	...
	for (const range of searchRange) { // loop through search ranges
		...
		const
			left = subRC.end.col < rangeRC.start.col, // if sub range's end is to the left of search range's start
...
function exists(sheet: string, cell: string) {
	try {
...
function returnFetch(url: string): string {
