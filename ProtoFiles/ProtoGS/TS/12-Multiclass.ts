...
function getClassInfo() { // returns level info
	...
	const cell = (row: number, col: number) => classRange.getCell(row, col).getValue()
...
function adjustNote(ss: GoogleAppsScript.Spreadsheet.Spreadsheet) {
	...
	for (let i = 1; i <= classes.getNumRows(); i++) { // loop through class range rows
		const cell = (row: number, col: number) => classes.getCell(row, col) // arrow function to return specific cell of class range
	...
	const noteBuilder = className.reduce(
		(str, x, j) => (str +
...
function doSpells(
	...
	if (spells != "None") // if the class's caster type is not 'None'
		if (!sheetNames.includes(`${Class} Spells`)) {
			...
			for (const protection of protections) newSheet.getRange(protection.getRange().getA1Notation()).protect().setWarningOnly(true)
